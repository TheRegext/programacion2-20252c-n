...
bool Alumno::operator==(const char *ap){
    if(strcmp(apellido, ap) == 0){
        return true;
    }
    return false;
}
bool Alumno::operator==(int d){
    if(dni == d){
        return true;
    }
    return false;
}
void Alumno::operator=(Fecha aux){
    fechaNacimiento = aux;
}
void Alumno::operator=(const char *n){
    strcpy(nombre, n);
}
bool Alumno::operator!=(const char *n){
...
void AlumnoNueva::operator=(Alumno obj){
